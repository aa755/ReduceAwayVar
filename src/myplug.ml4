 ...
 *)
  ...
  | Term.Prod (y, s, t) -> occurCheck x trm (let (b1, n1) = find env true x s in
  ...
  | Term.App (s, ts) -> occurCheck x trm (let (b1, n1) = find env true x s in
                                   let (b2, n2) = CArray.fold_map (fun b t -> let (b2, n2) = find env true x t in
                                                                                 (b ||b2, n2))  false ts in
                                   redB env b (b1 || b2) false (Term.mkApp (n1, n2)))
  | Term.Lambda (y, typ, t2) -> occurCheck x trm (let (b1, n1) = find env true x typ in
  ...
  | Term.LetIn (y, s, typ, u) -> occurCheck x trm (let (b1, n1) = find env true x s in
  ...
  | Term.Case (i, s, t, us) -> occurCheck x trm (*the branches are lambdas. so no need to add to the typing context*) 
  ...
  | Term.Proj (y, z) -> occurCheck x trm ( redB env b true true z)
  | Term.Cast (s, k, t) -> occurCheck x trm ( (let (b1, n1) = find env true x s in
                                  let (b2, n2) = find env true x t in
  ...
  | Term.CoFix  (y, (name_array, type_array, term_array)) -> occurCheck x trm (
    let (b2, n2) = CArray.fold_map (fun b u -> let (b3, n3) = find env true (x + CArray.length name_array) u in
                                               (b ||b3, n3))  false type_array in
    let (b3, n3) = CArray.fold_map (fun b u -> let (b3, n3) = find env true (x + CArray.length name_array) u in
                                               (b ||b3, n3))  false term_array 
